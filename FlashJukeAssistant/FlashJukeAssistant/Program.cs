...
namespace FlashJukeAssistant
{
    class Program
    {
        ...
        {
            double setDist = 30;
            bool rangeCond = (((Spots[flashSpot][0] - ObjectManager.Player.Position.X) * (Spots[flashSpot][0] - ObjectManager.Player.Position.X))
            + ((Spots[flashSpot][2] - ObjectManager.Player.Position.Y) * (Spots[flashSpot][2] - ObjectManager.Player.Position.Y)) < setDist * setDist);
            if (rangeCond)
            {
                return true;
            }
        ...
        public static bool checkForEnemyPlayer()
        {
            foreach (var Hero in ObjectManager.Get<Obj_AI_Hero>())
            {
                ...
                && NavMesh.IsWallOfGrass(Hero.Position, 1.0f)
                && (((SpotsFinal[flashSpot][0] - Hero.Position.X) * (SpotsFinal[flashSpot][0] - Hero.Position.X))
                + ((SpotsFinal[flashSpot][2] - Hero.Position.Y) * (SpotsFinal[flashSpot][2] - Hero.Position.Y)) < setdist1 * setdist1))
                {
                    return true;
                }
            }
        ...
        {
            foreach (var Object in ObjectManager.Get<Obj_AI_Minion>())
            {
                if (Object.Team != ObjectManager.Player.Team
                && NavMesh.IsWallOfGrass(Object.Position, 1.0f)
                && (((SpotsFinal[flashSpot][0] - Object.Position.X) * (SpotsFinal[flashSpot][0] - Object.Position.X))
                + ((SpotsFinal[flashSpot][2] - Object.Position.Y) * (SpotsFinal[flashSpot][2] - Object.Position.Y)) < setdist1 * setdist1))
                {
                    return true;
                }
            }
        ...
        public static bool checkForFriendlyVisionWard()
        {
            foreach (var Object in ObjectManager.Get<Obj_AI_Minion>())
            {
                if (Object.Team == ObjectManager.Player.Team
                && NavMesh.IsWallOfGrass(Object.Position, 1.0f)
                && Object.Name.ToUpper().Contains("VISION")
                && (((SpotsFinal[flashSpot][0] - Object.Position.X) * (SpotsFinal[flashSpot][0] - Object.Position.X))
                + ((SpotsFinal[flashSpot][2] - Object.Position.Y) * (SpotsFinal[flashSpot][2] - Object.Position.Y)) < setdist1 * setdist1))
                {
                    return true;
